...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['bulk_delete'])) {
        ...
        if (isset($_GET['filter']))
            $redirect .= (strpos($redirect, '?') !== false ? '&' : '?') . 'filter=' . $_GET['filter'];
        if (isset($_GET['service']))
            $redirect .= (strpos($redirect, '?') !== false ? '&' : '?') . 'service=' . $_GET['service'];
        if (isset($_GET['search']))
            $redirect .= (strpos($redirect, '?') !== false ? '&' : '?') . 'search=' . $_GET['search'];
    ...
    if (isset($_POST['enable_2fa_step1'])) {
        $ga = new GoogleAuthenticator();
        $secret = $ga->createSecret();
        $qrCodeUrl = $ga->getQRCodeGoogleUrl('KVIL Panel', $secret);
        echo json_encode(['secret' => $secret, 'qr_url' => $qrCodeUrl]);
        exit;
    }

    if (isset($_POST['enable_2fa_confirm'])) {
        $secret = $_POST['secret'];
        $code = $_POST['code'];
        $ga = new GoogleAuthenticator();
        if ($ga->verifyCode($secret, $code)) {
            $stmt = $db->prepare("UPDATE users SET secret_2fa = :secret WHERE id = 1");
    ...
    if (isset($_POST['disable_2fa'])) {
        $db->exec("UPDATE users SET secret_2fa = NULL WHERE id = 1");
        header("Location: check.php?page=settings");
        exit;
    }

    if (isset($_POST['verify_2fa_login'])) {
        $code = $_POST['2fa_code'];
        $stmt = $db->query("SELECT secret_2fa FROM users WHERE id = 1");
    ...
    if (isset($_POST['password'])) {
        ...
        $stmt = $db->query("SELECT password, secret_2fa FROM users WHERE id = 1");
    ...
    <body>
        <div class="login-container">
            ...
            <?php if (isset($_SESSION['2fa_pending'])): ?>
                <form method="POST">
                    <input type="text" name="2fa_code" placeholder="Enter 2FA Code" required autocomplete="off" autofocus>
                    <button type="submit" name="verify_2fa_login">Verify</button>
                    <button type="submit" name="logout" style="background: #ef4444; margin-top: 5px;">Cancel</button>
                </form>
            <?php else: ?>
                <form method="POST">
...
<body>
    <div class="sidebar">
        ...
        <a href="?page=dashboard&filter=all"
            class="nav-item <?php echo $page === 'dashboard' && $filter === 'all' ? 'active' : ''; ?>">
            <span class="icon">📊</span>
            <span>Dashboard</span>
            <span class="count"><?php echo $counts['all']; ?></span>
        </a>
        <a href="?page=dashboard&filter=new"
            class="nav-item <?php echo $page === 'dashboard' && $filter === 'new' ? 'active' : ''; ?>">
            <span class="icon">🆕</span>
            <span>New</span>
            <span class="count"><?php echo $counts['new']; ?></span>
        </a>
        <a href="?page=dashboard&filter=verified"
            class="nav-item <?php echo $page === 'dashboard' && $filter === 'verified' ? 'active' : ''; ?>">
            <span class="icon">✓</span>
            <span>Verified</span>
            <span class="count"><?php echo $counts['verified']; ?></span>
        </a>
        <a href="?page=dashboard&filter=favorites"
            class="nav-item <?php echo $page === 'dashboard' && $filter === 'favorites' ? 'active' : ''; ?>">
            <span class="icon">⭐</span>
            <span>Favorites</span>
            <span class="count"><?php echo $counts['favorites']; ?></span>
        </a>
        <a href="?page=dashboard&filter=unverified"
            class="nav-item <?php echo $page === 'dashboard' && $filter === 'unverified' ? 'active' : ''; ?>">
            <span class="icon">✗</span>
            ...
            <?php if ($page === 'settings'): ?>
                <div class="details-panel">
                    <div class="details-section">
                        ...
                        <form method="POST" class="settings-form">
                            <div class="form-group">
                                ...
                                <input type="text" name="telegram_bot_token" placeholder="Enter your Telegram bot token"
                                    value="<?php echo htmlspecialchars($telegram_bot_token); ?>">
                            </div>
                            <div class="form-group">
                                <label>Telegram Chat ID</label>
                                <input type="text" name="telegram_chat_id" placeholder="Enter your Telegram chat ID"
                    ...
                    <div class="details-section">
                        ...
                        <?php
                        $stmt = $db->query("SELECT secret_2fa FROM users WHERE id = 1");
                        $secret_2fa = $stmt->fetchColumn();
                        ?>

                        <div id="2fa-section">
                            <?php if ($secret_2fa): ?>
                                <p style="margin-bottom: 15px; color: #10b981;">✅ Two-Factor Authentication is ENABLED</p>
                                <form method="POST" onsubmit="return confirm('Are you sure you want to disable 2FA?');">
                                    <button type="submit" name="disable_2fa" class="btn-delete">Disable 2FA</button>
                                </form>
                            <?php else: ?>
                                <p style="margin-bottom: 15px; color: rgba(255,255,255,0.7);">Two-Factor Authentication adds an
                                    extra layer of security to your account.</p>
                                <button type="button" class="btn-save" onclick="start2FASetup()" id="btn-setup-2fa">Setup
                                    2FA</button>

                                <div id="2fa-setup-area"
                                    style="display: none; margin-top: 20px; background: rgba(0,0,0,0.2); padding: 20px; border-radius: 8px;">
                                    <div style="text-align: center; margin-bottom: 20px;">
                                        <img id="2fa-qr" src="" alt="QR Code"
                                            style="background: white; padding: 10px; border-radius: 4px;">
                                        <p style="margin-top: 10px; font-family: monospace; font-size: 16px; color: #fff;"
                                            id="2fa-secret-display"></p>
                                        <p style="font-size: 12px; color: rgba(255,255,255,0.5);">Scan with Google Authenticator
                                            or generic TOTP app</p>
                                    </div>
                                    <div class="form-group">
                                        <label>Verify Code</label>
                                        <input type="text" id="2fa-verify-input" placeholder="Enter 6-digit code">
                                    </div>
                                    <button type="button" class="btn-save" onclick="confirm2FA()">Enable 2FA</button>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <script>
                            ...
                            function start2FASetup() {
                                ...
                                formData.append('enable_2fa_step1', '1');
                                ...
                                fetch('check.php', {
                                    ...
                                    .then(data => {
                                        ...
                                        document.getElementById('2fa-qr').src = data.qr_url;
                            ...
                            function confirm2FA() {
                                const code = document.getElementById('2fa-verify-input').value;
                                if (!code) return alert('Please enter code');

                                const formData = new FormData();
                                formData.append('enable_2fa_confirm', '1');
                                formData.append('secret', tempSecret);
                    ...
                    <?php if (count($accounts) > 0): ?>
                        <?php foreach ($accounts as $account): ?>
                            <div class="account-item <?php echo $selected_account && $selected_account['id'] == $account['id'] ? 'active' : ''; ?>"
                                ...
                                <input type="checkbox" class="account-checkbox" data-account-id="<?php echo $account['id']; ?>"
                        ...
                        <div class="bulk-actions" id="bulk-actions">
                            ...
                            <div style="display: flex; gap: 10px; align-items: center;">
                                ...
                                <form method="POST" id="bulk-delete-form" style="display: inline;"
                                    onsubmit="return confirm('Are you sure you want to delete the selected accounts? This action cannot be undone.');">
                    ...
                    <?php if ($selected_account): ?>
                        <div class="details-section">
                            ...
                            <div class="info-row">
                                ...
                                <div class="info-value"><?php echo htmlspecialchars($selected_account['ip_address'] ?: '-'); ?>
                                </div>
                            </div>
                            <div class="info-row">
                            ...
                            <div class="info-row">
                                ...
                                <div class="info-value">
                                    <form method="POST" style="display: inline;">
                                        ...
                                        <button type="submit" name="toggle_verified" class="status-toggle-btn"
                                            style="background: <?php echo $selected_account['verified'] ? '#10b981' : '#ef4444'; ?>; color: white;">
                                            <?php echo $selected_account['verified'] ? '✓ Verified' : '✗ Unverified'; ?>
                                        </button>
                                    </form>
                                    <form method="POST" style="display: inline;">
                                        <input type="hidden" name="account_id" value="<?php echo $selected_account['id']; ?>">
                                        <button type="submit" name="toggle_favorite" class="status-toggle-btn"
                                            style="background: <?php echo $selected_account['favorite'] ? '#fbbf24' : '#6b7280'; ?>; color: white;">
                        ...
                        <?php if ($selected_account['cookie_script']): ?>
                            <div class="details-section">
                                ...
                                <div class="action-buttons">
                                    ...
                                    <button class="action-btn btn-send-tg"
                                        onclick="sendToTG(<?php echo $selected_account['id']; ?>, this)">Send to TG</button>
                                </div>
                            </div>
                        <?php endif; ?>
                    <?php else: ?>
